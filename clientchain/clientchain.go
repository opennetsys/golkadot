...
func (c *Chain) InitGenesisFromBest(bestHeader *types.Header, rollback bool) *clientchaintypes.ChainGenesis {
	hexState := u8util.ToHex(bestHeader.StateRoot.Value(), 48, true)
	fmt.Printf("Initializing from state %s", hexState)

	c.State.DB.SetRoot(bestHeader.StateRoot.Value())
...
// RollbackBlock ...
// NOTE: the default for rollback bool should be true
func (c *Chain) RollbackBlock(bestHeader *types.Header, rollback bool) *clientchaintypes.ChainGenesis {
	prevHash := bestHeader.ParentHash.Value()
	prevNumber := bestHeader.BlockNumber().Value() - 1
	...
	if rollback && prevNumber > 1 {
		...
		c.Blocks.BestNumber.Set(types.NewInt(prevBlock.Header.BlockNumber().Value(), 64))
...
func (c *Chain) CreateGenesisState() {
	...
	c.State.DB.Transaction(func() bool {
